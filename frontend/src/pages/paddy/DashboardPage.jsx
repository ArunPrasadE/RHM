...
export default function PaddyDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      const fieldsData = await api.get('/paddy/fields');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Quick Actions */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Link to="/paddy/fields" className="card hover:shadow-lg transition-shadow flex items-center gap-3">
          <div className="w-10 h-10 bg-green-100 dark:bg-green-900/50 rounded-lg flex items-center justify-center">
            <FieldIcon className="w-5 h-5 text-green-600 dark:text-green-400" />
          </div>
          <span className="font-medium">Manage Fields</span>
        </Link>
        <Link to="/paddy/workers" className="card hover:shadow-lg transition-shadow flex items-center gap-3">
        ...
        <Link to="/paddy/expenses" className="card hover:shadow-lg transition-shadow flex items-center gap-3">
          <div className="w-10 h-10 bg-red-100 dark:bg-red-900/50 rounded-lg flex items-center justify-center">
            <ExpenseIcon className="w-5 h-5 text-red-600 dark:text-red-400" />
          </div>
          <span className="font-medium">Add Expense</span>
        </Link>
        <Link to="/paddy/income" className="card hover:shadow-lg transition-shadow flex items-center gap-3">
          <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/50 rounded-lg flex items-center justify-center">
            <IncomeIcon className="w-5 h-5 text-amber-600 dark:text-amber-400" />
          </div>
