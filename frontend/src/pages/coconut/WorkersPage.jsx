...
export default function WorkersPage() {
  ...
  const fetchData = async () => {
    try {
      const [workersData, categoriesData] = await Promise.all([
        ...
        api.get('/coconut/workers/categories/list')
      ]);
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      await api.post('/coconut/workers/categories', newCategory);
  ...
  const handleUpdateCategory = async (e) => {
    ...
    try {
      await api.put(`/coconut/workers/categories/${editingCategory.id}`, {
  ...
  const handleDeleteCategory = async (category) => {
    ...
    try {
      await api.delete(`/coconut/workers/categories/${category.id}`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg text-sm font-medium bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300 hover:bg-amber-200 dark:hover:bg-amber-800"
        >
          Manage Categories
        </button>
      </div>
