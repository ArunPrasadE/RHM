...
export default function GroveForm({ grove, onSave, onClose }) {
  const [formData, setFormData] = useState({
    name: grove?.name || '',
    area_cents: grove?.area_cents || '',
    gps_latitude: grove?.gps_latitude || '',
    gps_longitude: grove?.gps_longitude || '',
    google_maps_url: grove?.google_maps_url || ''
  });
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold">
              {grove ? 'Edit Grove (தோப்பு திருத்து)' : 'Add New Grove (புதிய தோப்பு சேர்)'}
            </h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="label">Grove Name (தோப்பு பெயர்) *</label>
              <input
                type="text"
                value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                className="input"
                placeholder="e.g., Periya Thoppu"
                required
              />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {loading ? 'Saving...' : grove ? 'Update Grove' : 'Add Grove'}
              </button>
            </div>
          </form>
        </div>
